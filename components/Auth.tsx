...
export const LoginForm: React.FC<AuthFormProps> = ({ onSuccess }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {error && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-full font-sans text-sm">
          {error}
        </div>
      )}
      <div>
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border border-gray-300 rounded-full font-sans text-sm text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50"
      >
        {loading ? (
          <SpinnerIcon />
        ) : (
